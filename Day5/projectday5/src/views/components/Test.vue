    ...
    <table class="task-table">
      ...
      <tbody>
        <tr v-for="(task, index) in tasks" :key="index" class="hover:bg-green-100 h-12 text-black text-base">
          ...
          <td class="border px-4 py-2 text-center">{{ task.date }}</td>
          <td class="border px-4 py-2 text-center">
            <Button :title="'Edit'" @click="editTask(index)" class="text-sm py-1 font-normal m-0" />
          </td>
          <td class="border px-4 py-2 text-center">
            <div class="flex justify-center items-center">
              <Checkbox :checked="checkedTasks.includes(task.id)" @change="toggleCheck(task.id)" />
            </div>
          </td>
        </tr>
      </tbody>
    </table>
    <Button title="Show Completed" @click="showCompletedTasks" class="mt-3" />
...
const showCompletedTasks = () => {
  completedTasks.value = props.tasks.filter(task => task.checked);
};
</script>
